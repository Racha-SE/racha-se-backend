...
- `src/utils/auth.ts` builds the single `auth` instance (config + drizzle adapter). `src/routes/auth.route.ts` mounts it: `.mount(auth.handler)` with **no path argument** — better-auth's own `basePath` already resolves to `/api/auth`, passing a path adds an _extra_ prefix on top instead of matching it (verified: `.mount('/auth', handler)` serves at `/auth/api/auth`, not `/auth`).
- `authRoute` is `.use()`'d on a plain top-level `Elysia()` instance in `src/index.ts`, not inside the `{ prefix: "/v1" }` group — chaining `.mount()` under a prefixed instance shadows the raw handler (404s on both the prefixed and unprefixed path). Verified empirically; not documented behavior.
